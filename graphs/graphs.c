...
void dfs_scc(int at,Stack *stack,int *onStack,int *ids,int *low,int *id,graph *g,scc_list *res) {
    ...
    while (curr != NULL) {
        if (ids[curr->val] == unvisited) { dfs_scc(curr->val, stack, onStack, ids, low, id, g,res); }
